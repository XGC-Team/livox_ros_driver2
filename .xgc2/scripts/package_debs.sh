...
VERSION="${PACKAGE_VERSION:-1.2.6-1}"
ARCH="$(dpkg --print-architecture)"
PACKAGE="ros-noetic-livox-ros-driver2"
ROS_PACKAGE="livox_ros_driver2"
...
if [[ ! -x "${pkg_root}${PREFIX}/lib/${ROS_PACKAGE}/livox_ros_driver2_node" ]]; then
  echo "missing installed livox_ros_driver2_node" >&2
  exit 1
fi
...
Description: Livox ROS Driver 2 for ROS Noetic
 ROS1 driver node, launch files, configuration, and custom messages for
 Livox HAP and Mid-360 class devices.
EOF
...
chmod 0755 "${pkg_root}${PREFIX}/lib/${ROS_PACKAGE}/livox_ros_driver2_node"
